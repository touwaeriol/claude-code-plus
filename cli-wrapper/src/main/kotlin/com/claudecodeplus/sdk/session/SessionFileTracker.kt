...
class SessionFileTracker(
     ...
     */
    suspend fun readNewMessages(): List<ClaudeFileMessage> = mutex.withLock {
     ...
     * @return 文件中的所有消息
     */
    suspend fun readAllMessages(): List<ClaudeFileMessage> = mutex.withLock {
        withContext(Dispatchers.IO) {
            val file = File(filePath)
            if (!file.exists()) {
                logger.debug { "Session file does not exist: $filePath" }
     ...
     * 从指定行开始读取消息
     */
    private fun readMessagesFromLine(file: File, startLine: Int): List<ClaudeFileMessage> {
    ...
    private fun parseMessage(line: String): ClaudeFileMessage? {
        ...
        return try {
            val message = gson.fromJson(line, ClaudeFileMessage::class.java)
