...
1. **命令发现和执行**：自动查找系统中的 `claude` 命令位置
2. **参数映射**：将 QueryOptions 转换为 Claude CLI 参数
   - `--append-system-prompt` 代替了旧的 `--custom-system-prompt`
   - `--mcp-config` 代替了旧的 `--mcp-servers`
   - 使用 `--print` 参数启用非交互模式
3. **流式响应处理**：解析 claude CLI 的 JSONL 流输出
4. **会话管理**：处理会话 ID 和恢复功能
5. **错误处理**：捕获并处理进程错误和异常
6. **中断支持**：通过 `terminate()` 方法终止进程
...
private fun findClaudeCommand(): String {
    return when {
        isWindows() -> findWindowsClaudeCommand()
        else -> findUnixClaudeCommand()
    }
}
