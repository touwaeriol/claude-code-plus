...
### 2025-06-30 模型选择器功能恢复

完成了模型选择功能的重新实现，将其移至输入框左下角：

1. **模型选择器重新设计**
   - 移除了ChatHeader中的原有模型选择器
   - 在EnhancedSmartInputArea的左下角添加了CompactModelSelector
   - 使用更小、更淡的字体显示模型名称
   - 模型名称后跟"＜"符号，符合用户要求

2. **模型支持简化**
   - 仅支持两个模型：Claude 4 Opus（对应CLI参数"opus"）和Claude 4 Sonnet（对应CLI参数"sonnet"）
   - 点击模型选择器可在两个模型之间切换
   - 模型参数正确传递给ClaudeCliWrapper
...
3. **功能恢复状态**
   ...
   - ✅ "Generating..."指示器 - 已完成
   - ✅ 发送按钮样式 - 已完成
   - ⚠️ @ 符号上下文菜单功能 - 已简化（需要进一步恢复完整功能）
   - ⚠️ ANSI序列处理 - 需要进一步验证

4. **代码清理**
   - 删除了未使用的重复组件
   - 简化了EnhancedSmartInputArea的实现
   - 修复了编译错误和语法问题

### 2025-06-30 Jewel组件升级和UI改进

进一步优化了UI组件，全面迁移到Jewel组件：

1. **输入框组件升级**
   - 使用Jewel TextArea替代BasicTextField
   - 更好的键盘处理和多行支持
   - 原生的IntelliJ主题集成

2. **模型选择器改进**
   - 修改符号从"＜"改为"⌃"，更符合UI设计规范
   - 添加下拉选择菜单，用户可以点击查看所有模型选项
   - 支持点击选择而不是简单的循环切换
   - 下拉菜单使用Jewel样式，与整体主题保持一致

3. **交互体验优化**
   - TextArea提供更好的文本编辑体验
   - 改进的placeholder显示
   - 更自然的键盘快捷键支持（Enter发送，Shift+Enter换行）
   - 模型选择更直观，用户可以看到所有可用选项

4. **技术改进**
   - 完全使用Jewel UI组件，减少对原生Compose组件的依赖
   - 更好的类型安全（TextFieldValue vs String）
   - 统一的主题颜色管理
   - 添加了流式响应时的光标效果（▌）
...
- 实现 JewelConversationView：纯 Compose 组件，基于 ClaudeCliWrapper
- 实现 JewelChatPanel：Swing 包装器，集成到 IDEA 平台
- 创建 JewelChatApp：独立桌面应用测试
...
## 2025-06-24 上下文引用输入优化

优化了工具窗口中的 @ 上下文引用输入体验：

1. **创建 EnhancedSmartInputArea 组件**
   - 支持内联显示上下文引用（`@type://value` 格式）
   - 使用 `TextFieldValue` 跟踪光标位置，实现精确的输入控制
   - 实现了 `VisualTransformation` 为上下文引用添加蓝色背景高亮

2. **触发机制优化**
   - **只有独立的 @ 符号**（前后都没有字符）才会触发上下文菜单
   - 例如：`输入 @ 这里` 会触发，但 `test@example.com` 不会触发
   - 支持识别多种引用格式：file、folder、symbol、terminal、problems、git、code、http(s)、selection、workspace

3. **引用格式标准化**
   - 所有上下文引用使用 `@type://` 的 URI 格式
   - 当前支持：`@file://`、`@folder://`、`@symbol://`、`@terminal://`、`@problems://`、`@git://`
   - 未来支持：`@code://`、`@http://`、`@https://`、`@selection://`、`@workspace://`

4. **交互特性**
   - 上下文引用以**蓝色背景**高亮显示
   - 支持**整体删除**：删除引用时会删除整个 `@type://value` 字符串
   - 在输入上下文引用期间，用户可以继续输入或删除字符来修改搜索条件
   - ESC 键关闭建议菜单，Shift+Enter 换行，Enter 发送消息

5. **更新的文档**
   - 更新了 `TOOLWINDOW_REDESIGN.md`，详细说明了触发机制和引用格式
   - 明确了当前支持和未来计划支持的上下文类型

6. **调试支持**
   - 添加了调试日志用于诊断 Claude CLI 通信问题
   - 记录消息发送、接收和处理的详细信息

### 2025-07-01 模型选择功能修复与UI优化

完成了模型选择功能的全面修复和UI重构工作：

1. **模型选择功能修复**
   - **修复硬编码问题**：解决了 sendMessage 函数中模型参数被硬编码为 "sonnet" 的Bug
   - **正确参数传递**：确保选择的模型参数正确传递到 ClaudeCliWrapper 和 Claude CLI
   - **默认模型设置**：将默认模型从 SONNET 改为 OPUS，符合用户需求
   - **模型映射验证**：验证了 AiModel.OPUS 的 cliName 为 "opus"，AiModel.SONNET 的 cliName 为 "sonnet"

2. **UI组件重构**
   - **使用 Jewel Popup**：用 Jewel 的 Popup 组件替换了自定义下拉实现
   - **向上展开设计**：模型选择器向上弹出，符合UI设计要求
   - **小巧无边框**：
     - 按钮尺寸：padding 从 (8dp,4dp) 减小到 (6dp,3dp)
     - 字体大小：模型名称从 11sp 减小到 10sp，箭头从 10sp 减小到 8sp
     - 弹出框：固定宽度 120dp，移除所有边框
     - 间距优化：元素间距从 4dp 减小到 3dp

3. **主题一致性**
   - **背景色统一**：使用 `JewelTheme.globalColors.panelBackground`
   - **文字色统一**：使用 `JewelTheme.globalColors.text.normal/disabled`
   - **无边框设计**：完全移除边框，与主题背景完美融合
   - **选中状态**：高亮透明度调整为 0.15f，更明显的视觉反馈

4. **功能验证**
   - **模型切换正常**：测试确认 Opus ↔ Sonnet 切换功能正常
   - **CLI参数正确**：验证传递到 Claude CLI 的模型参数为 "claude-opus-4-20250514"
   - **状态同步**：内部状态与外部状态正确同步
   - **调试信息**：保留完整的调试输出以便后续维护

5. **代码提交**
   - **功能修复提交**：【刘雄风】fix：修复模型选择功能硬编码问题并设置默认模型为Opus
   - **UI重构提交**：【刘雄风】refactor：重构模型选择器UI，使用Jewel Popup组件并优化小巧无边框设计
   - **文件清理**：删除了测试用的 find-class.sh 脚本

### 修改的文件列表
- `JewelChatApp.kt` - 添加模型选择支持，修复硬编码问题
- `JewelChatPanel.kt` - 默认模型改为 OPUS
- `JewelConversationView.kt` - 默认模型改为 OPUS  
- `EnhancedSmartInputArea.kt` - 重构 CompactModelSelector，使用 Popup 组件
- `ModelSelector.kt` - 调整模型切换逻辑顺序

### 技术特点
- **响应式设计**：使用 Compose 的状态管理实现响应式更新
- **组件化架构**：模型选择器独立封装，易于维护和复用
- **性能优化**：使用 remember 和 LaunchedEffect 避免不必要的重组
- **类型安全**：使用枚举类型确保模型参数的类型安全