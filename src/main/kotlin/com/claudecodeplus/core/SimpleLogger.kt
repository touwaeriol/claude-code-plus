 ...
 */
class IdeaLogger(private val logger: com.intellij.openapi.diagnostic.Logger) : SimpleLogger {
    override fun debug(message: String) {
...
object LoggerFactory {
    fun getLogger(clazz: Class<*>): SimpleLogger {
        return try {
            // 尝试使用 IDEA Logger
            val ideaLogger = com.intellij.openapi.diagnostic.Logger.getInstance(clazz)
            IdeaLogger(ideaLogger)
        } catch (e: Throwable) {
            // 回退到 Java Logger
            JavaLogger(clazz.name)
        }
    }
}