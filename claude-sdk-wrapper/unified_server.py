...
"""
Claude SDK 统一服务器
同时支持 HTTP 和 WebSocket 两种通信方式

特性：
- HTTP 接口：/stream (SSE 流式响应)、/message (单次请求响应)
- WebSocket 接口：/ws (实时双向通信)
- 共享会话管理
...
class ClaudeService:
    ...
    def initialize(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """初始化服务"""
        try:
            logger.info(f"[DEBUG] Initializing with config: {config}")
            if 'cwd' in config:
                logger.info(f"[DEBUG] Initialize cwd: {config['cwd']}")
    ...
    async def stream_message(self, message: str, session_id: str, custom_options: Optional[Dict[str, Any]] = None):
        ...
        # 调试：记录收到的 custom_options
        if custom_options:
            logger.info(f"[DEBUG] Received custom options: {custom_options}")
            if 'cwd' in custom_options:
                logger.info(f"[DEBUG] Working directory (cwd): {custom_options['cwd']}")
            else:
                logger.warning("[DEBUG] No cwd in custom options")
    ...
    async def get_single_response(self, message: str, session_id: str, custom_options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """获取单次完整响应"""
        response_chunks = []
        error = None
        
        async for chunk in self.stream_message(message, session_id, custom_options):
...
async def handle_message(request):
    ...
    try:
        ...
        if new_session:
            session_id = claude_service.create_session()
            logger.info(f"[/message] Creating new session as requested: {session_id}")
        elif not session_id:
            session_id = claude_service.get_or_create_default_session()
            logger.info(f"[/message] Using default session: {session_id}")
        
        logger.info(f"[/message] Request (session: {session_id}): {message[:100]}{'...' if len(message) > 100 else ''}")
        
        # 获取完整响应
        result = await claude_service.get_single_response(message, session_id, options)
        
        logger.info(f"[/message] Response: success={result.get('success')}")
...
async def websocket_handler(request):
    ...
    await ws.send_json({
        'type': 'welcome',
        'message': 'Connected to Claude Unified Server (WebSocket)',
        'initialized': claude_service.is_initialized
    })
...
def create_app():
    ...
    # HTTP 路由
    app.router.add_post('/stream', handle_stream)   # SSE 流式接口
    app.router.add_post('/message', handle_message) # 单次响应接口
    app.router.add_get('/health', handle_health)    # 健康检查
    
    # WebSocket 路由
    app.router.add_get('/ws', websocket_handler)    # WebSocket 接口
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Claude SDK Unified Server (HTTP + WebSocket)')
    ...
    logger.info(f"Starting unified server on {args.host}:{args.port}")
    logger.info(f"Claude SDK available: {CLAUDE_SDK_AVAILABLE}")
    logger.info("HTTP APIs:")
    ...
    logger.info("  WS   /ws      - Real-time bidirectional communication")
