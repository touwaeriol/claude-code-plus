...
object ClaudeSessionManager {
    ...
    private fun buildClaudeOptions(
        ...
        // 前端思考开关（默认为 true）
        val thinkingEnabled = sessionOptions?.get("thinkingEnabled")?.jsonPrimitive?.booleanOrNull ?: true
        ...
        // 项目路径（用于解析 Claude settings）
        val projectPath: Path? = ideActionBridge.getProjectPath()?.let { java.nio.file.Path.of(it) }

        // 加载并合并 Claude settings
